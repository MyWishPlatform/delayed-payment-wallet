...
const LostKeyERC20Wallet = artifacts.require('./LostKeyDelayedPaymentWallet.sol');
...
contract('LostKeyERC20Wallet', function (accounts) {
    ...
    it('#1 construct', async () => {
        const contract = await LostKeyERC20Wallet.new(TARGET, [TARGET], [100], 2 * MINUTE, 0, 0);
        contract.address.should.have.length(42);
    });

    it('#2 successfully sending funds', async () => {
        const contract = await LostKeyERC20Wallet.new(TARGET, [RECIPIENT_1], [100], MINUTE, 0, 0);
        await contract.sendTransaction({ value: web3.toWei(1, 'ether') });
        const balanceBefore = await getBalance(RECIPIENT_2);

        await contract.sendFunds(RECIPIENT_2, web3.toWei(0.5, 'ether'), '', { from: TARGET });
    ...
    it('#3 cannot send funds not by target', async () => {
        ...
        await contract.sendFunds(RECIPIENT_2, web3.toWei(0.5, 'ether'), '', { from: RECIPIENT_1 })
            .should.eventually.be.rejected;
    });

    it('#4 token balance', async () => {
        const wallet = await LostKeyERC20Wallet.new(TARGET, [RECIPIENT_1], [100], MINUTE, 0, 0);
    ...
    it('#8 cannot check before time', async () => {
        const contract = await LostKeyERC20Wallet.new(TARGET, [RECIPIENT_1, RECIPIENT_2], [50, 50], 10 * MINUTE, 0, 0);
        await contract.sendTransaction({ value: web3.toWei(1, 'ether') });
    ...
    it('#10 successfully sent funds after check', async () => {
        ...
        const contract = await LostKeyERC20Wallet.new(TARGET, recipients, percents, 10 * MINUTE, 0, 0);
    ...
    it('#11 cannot set not 100 percents', async () => {
        await LostKeyERC20Wallet.new(TARGET, [TARGET], [90], MINUTE, 0, 0).should.eventually.be.rejected;
        await LostKeyERC20Wallet.new(TARGET, [TARGET], [110], MINUTE, 0, 0).should.eventually.be.rejected;
    });
});
